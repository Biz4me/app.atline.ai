...
/**
 * AtlineSplash — écran de chargement animé, adapté au contexte :
 *  - Web mobile (navigateur / rafraîchissement) : VOYAGE — le A se dessine, le nœud orange
 *    remonte la ligne, étincelle + onde. (Animation riche.)
 *  - App installée (standalone) : GROW — le A démarre à la taille de l'icône de lancement puis
 *    grandit + signal (transition invisible avec l'icône OS).
 * Fond warm-white, puis fondu. Zéro dépendance.
 */
...
export function AtlineSplash({ minDuration = 1800 }: { minDuration?: number }) {
  ...
  return (
    ...
    >
      {isApp ? (
        /* ── APP installée : le A grandit depuis l'icône ── */
        <>
          ...
          >
            <g transform="translate(17.76,16.52) scale(0.62)">
              <path d="M16,92 L50,16 L84,92" fill="none" stroke={INK} strokeWidth="18" strokeLinejoin="round" strokeLinecap="round" />
              <polygon points="28,61 28,80 71,55.3 71,48.9" fill={INK} />
              <circle cx="71" cy="52.1" r="3.6" fill={INK} />
              <circle cx="86" cy="47" r="7" fill="none" stroke={ORANGE} strokeWidth="2.4" opacity="0">
                <animate attributeName="r" values="7;15" begin="0.55s" dur="0.55s" fill="freeze" />
                <animate attributeName="opacity" values="0.6;0" begin="0.55s" dur="0.55s" fill="freeze" />
              </circle>
              <circle cx="86" cy="47" r="9" fill={ORANGE}>
                <animate attributeName="r" values="9;11.5;9" begin="0.5s" dur="0.4s" fill="freeze" />
              </circle>
            </g>
          </svg>
        </>
      ) : (
        /* ── Web mobile : le A se dessine + le nœud voyage ── */

...
- **Coquille responsive UNIQUE des fils = `components/fil-shell.tsx` (`FilShell`)** : tout le responsive 3→2→1 vit LÀ, en CSS pur, piloté par le seul booléen `open` (le rail S2). Les 4 fils (contact/aria/nova/atlas) l'utilisent (Atlas importe `RAIL_CLS` + `CENTER_HIDDEN_WHEN_OPEN` car il garde sa hauteur propre). **Paliers (S1 dès `md`, 3ᵉ panneau à `xl`)** : `< md` (768) = 1 panneau (rail en overlay `fixed inset-0`) · `md→xl` = 2 panneaux (S1 340 + le rail REMPLACE le centre, `CENTER_HIDDEN_WHEN_OPEN = md:max-xl:hidden`) · `≥ xl` (1280) = 3 panneaux (S1 340 + centre + rail 400). `openInfo` = juste `setInfoOpen(true)` + `?info=1` (plus de `matchMedia`). Centre des fils unifié à `max-w-2xl` ; **contenu du rail plafonné `max-w-md`** (fiche embarquée + `AgentPanel`) pour ne pas s'étaler en 2 panneaux. Toute nouvelle surface « fil + panneau » passe par `FilShell`.
  - **DÉPENDANCE CRITIQUE** : `ChatsShell` (colonne S1) passe à `hidden md:block`, ET le cap « téléphone » `.app-shell { max-width: 480px }` de `globals.css` (+ `md:max-w-none md:mx-0` dans `app-shell.tsx`) est **levé dès `md` (768)** — sinon toute la messagerie est étranglée dans 480px en tablette. `/chats/page.tsx` (`md:hidden`/`hidden md:grid`), flèches retour des fils (`md:hidden`), composeur (`AppComposer` : bascule mobile→attaché à `md` UNIQUEMENT quand `desktop`). Pages outils : restent en `lg` (colonne AppShell `fixed hidden lg:block`), juste pleine largeur dès md. Vérifié live 500/768/900/1100/1280.
